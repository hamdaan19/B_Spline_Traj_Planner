...
double MAX_VELOCITY_LIMIT = 5; // Velocity in ms^-1
double MAX_ACCELERATION_LIMIT = 2; // Acceleration in ms^-2 
...
void getTrajMsg(std::vector<double> vec, GFunc G_obj, Spline3D H,  trajectory_msgs::MultiDOFJointTrajectory* msg);
...
Eigen::Vector3d getVelocity(double tau, Spline3D* H, GFunc* G_obj){
    auto deriv = H->derivatives(tau, 1);
    Eigen::Vector3d vel = deriv.col(1) * ( 1/G_obj->gradG(tau) );
...
Eigen::Vector3d getAcceleration(double tau, Spline3D* H, GFunc* G_obj){
    auto deriv = H->derivatives(tau, 2);
...
bool service_cb(mav_companion_api::TrajPlanner::Request &req, mav_companion_api::TrajPlanner::Response &res) {
    ...
    if ( !waypoints.empty() ) { // if my waypoints varible is not empty
        ...
        getTrajMsg(intervalVector, optG, s, &traj_msg); 
...
_________________________________________________________________
 ...
 Function to create Trajectory message
_________________________________________________________________
...
void getTrajMsg(std::vector<double> vec, GFunc G_obj, Spline3D H,  trajectory_msgs::MultiDOFJointTrajectory* msg){
    ...
    msg->joint_names.push_back("body");

    double T = G_obj.G(1); // Final Time

    for (double t = 0; t <= T; t += 1e-2){
        double tau = G_obj.invertedG(t); 

        trajectory_msgs::MultiDOFJointTrajectoryPoint pt; 

        auto vel = getVelocity(tau, &H, &G_obj); 
        auto acc = getAcceleration(tau, &H, &G_obj); 

        geometry_msgs::Transform tf;  

        tf.translation.x = H(tau)[0];
        tf.translation.y = H(tau)[1];
        tf.translation.z = H(tau)[2];
        tf.rotation.x = 0; 
        tf.rotation.y = 0;
        tf.rotation.z = 0; 
        tf.rotation.w = 1;
        ...
        velMsg.linear.x = vel[0];
        velMsg.linear.y = vel[1];
        velMsg.linear.z = vel[2];

        pt.velocities.push_back(velMsg);

        geometry_msgs::Twist accMsg;

        accMsg.linear.x = acc[0];
        accMsg.linear.y = acc[1];
        accMsg.linear.z = acc[2];

        pt.accelerations.push_back(accMsg);
        ros::Duration t_from_start(t); 
        pt.time_from_start = t_from_start; 

        msg->points.push_back(pt); 
    }

    std::cout << "No. of temporal points: : " << msg->points.size() << std::endl;
